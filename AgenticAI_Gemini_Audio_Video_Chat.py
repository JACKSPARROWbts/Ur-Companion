        ...
        if self.session:
            ...
            if time.time() - self.last_frame_time > 1:
                ...
                if self.latest_args[2] is not None:
                    print("sending text")
                    print(self.latest_args[2])

                    await self.session.send(input=encode_text(self.latest_args[2]))
    ...
    with gr.Row() as row:
        ...
        with gr.Column():
            ...
            text_input=gr.Textbox(
                label="Text", type="text", placeholder="Type your message here...",
                lines=5, max_lines=10, show_copy_button=True,
            )

        webrtc.stream(
            GeminiHandler(),
            inputs=[webrtc, image_input,text_input],
            outputs=[webrtc],
